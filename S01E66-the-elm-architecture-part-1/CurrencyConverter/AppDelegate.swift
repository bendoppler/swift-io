...
struct Converter: Codable {
    ...
    init(inputText: String, rate: Double, currency: String) {
    ...
    var viewController: ViewController<State.Message> {
        ...
        if let r = rates {
            let cells: [TableViewCell<Message>] = r.keys.sorted().map { currency in
                return TableViewCell(identity: currency, text: currency, onSelect: Message.currencySelected(currency), onDelete: nil)
            }
            viewController = ViewController.tableViewController(TableView(items: cells))
        } else {
            viewController = ViewController.viewController(View.label(text: "No rates loaded", font: .systemFont(ofSize: 20)))
        }
        var viewControllers = [
            NavigationItem(title: "Rates",
                           ...
                           leftItemsSupplementsBackButton: false,
