...
extension BinaryTree {
    ...
    var bounds: CGRect {
        let width: CGFloat = pow(2, CGFloat(maxHeight) - 1) * nodeSize.width * spacing.horizontal
        let height: CGFloat = CGFloat(maxHeight) * nodeSize.height * spacing.vertical
    ...
    func render(into context: CGContext, at center: CGPoint, currentHeight: Int) {
        ...
        func recurse(child: BinaryTree, offset: CGFloat) {
            ...
            let childCenter: CGPoint = CGPoint(x: center.x + offset, y: center.y + nodeSize.height * spacing.vertical)
