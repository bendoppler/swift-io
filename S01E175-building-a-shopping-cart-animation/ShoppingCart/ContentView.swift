...
/// This key taks the first non-`nil` value
            ...
            content
                .overlayWithAnchor(value: .point(CGPoint(x: translation.width, y: translation.height)), transform: { anchor in
                    Color.white.opacity(0.00001)
                        ...
                        .highPriorityGesture(DragGesture().updating(
                            $state,
...
extension View {
    ...
    func onGeometryChange<Value: Equatable>(compute: @escaping (GeometryProxy) -> Value, onChange: @escaping (Value) -> ()) -> some View {
            ...
            HStack {
                ForEach(0..<colors.count) { index in
                    ShoppingItem(index: index)
                        .draggable(
                            snapBack: !isInDropZone,
                            onDragged: { frame in
                                self.dragRect = frame
                            },
                            onTapped: { anchor in
                                cartItems.append((index: index, anchor: anchor))
                            }, onEnded: { anchor in
                                guard isInDropZone else { return }
                                cartItems.append((index, anchor))
                            }
                        )
                }
            }.zIndex(1)
