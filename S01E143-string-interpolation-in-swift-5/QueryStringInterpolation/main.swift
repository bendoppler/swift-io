...
extension Query {
    func appending(_ other: QueryPart) -> Query<A> {
        return Query<A>(query.appending(other));
    }
}
...
extension QueryPart {
    ...
    var values: [SQLValue] {
        return parts.compactMap { part in
            guard case let .value(v) = part else {
                return nil
            }
            return v
        }
...
let tableName = "users"
let sample = Query<String>("SELECT * FROM \(raw: tableName) WHERE id=\(param: id) AND email=\(param: email)")
...
dump(sample2)
dump(sample2.query.sql)
dump(sample2.query.values)
