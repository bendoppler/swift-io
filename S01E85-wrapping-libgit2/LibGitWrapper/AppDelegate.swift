...
func wrap(_ fn: () -> Int32) throws {
    let result = fn()
    guard result == 0 else {
        let err: UnsafePointer<git_error>? = giterr_last()
        let message: String = String(cString: err!.pointee.message)
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        git_libgit2_init()
        let path = "/Users/dothaibao/Developer/libgit2/testrepo"
        do {
            let repo: Repository = try Repository(open: path)
            let reference: Reference = try Reference(repository: repo, dwim: "master")
