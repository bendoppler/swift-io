        ...
        func decode<T>(_ type: T.Type, forKey key: Key) throws -> T where T : Decodable {
            ...
            if type == URL.self {
                guard let href = el.attribute(forName: "href")?.stringValue else {
                    throw DecodingError.dataCorruptedError(forKey: key, in: self, debugDescription: "Expected href attributed")
                }
                ...
                let str = el.stringValue! // todo verify assumption
                guard let date = DateFormatter.isoDate.date(from: str) else {
                    throw DecodingError.dataCorruptedError(forKey: key, in: self, debugDescription: "Malformed date: \(str)")
                }
                return date as! T
            }
            else {
...
let account: Account = try decodeXML(xml)
print(account)
