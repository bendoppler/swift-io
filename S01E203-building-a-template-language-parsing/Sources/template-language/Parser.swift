...
extension AnnotatedExpression {
    public var simple: SimpleExpression {
        return SimpleExpression(expression: expression.map { $0.simple })
    }
}
...
extension Substring {
    ...
    func err(_ reason: ParseError.Reason) -> ParseError {
        return ParseError(reason: reason, offset: startIndex)
    }
    ...
    mutating func parse() throws -> AnnotatedExpression {
        ...
        if remove(prefix: "{") {
            ...
            while !isEmpty {
                ...
                guard first?.isAttributeName == true else {
                    throw err(.expected("Attribute or >"))
                }
