...
class PlayViewController: UIViewController, UITextFieldDelegate, AVAudioPlayerDelegate {
	...
	@objc func playerChanged(notification: Notification) {
		guard let r = notification.userInfo?["recording"] as? Recording?, let p = notification.userInfo?["state"] as? SharedPlayer.PlayerState else {
			return
		}
		updateForChangedRecording(r, state: p)
	}
