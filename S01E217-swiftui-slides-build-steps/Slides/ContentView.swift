...
//  Slides
//
//  Created by Do Bao on 11.16.2022.
//  Copyright © 2022 Do Bao. All rights reserved.
//
...
struct Presentation<S: SlideList, Theme: ViewModifier>: View {
    ...
    var context: Context {
        return Context(currentStep: currentStep, currentSlide: currentSlide, slideCount: slides.count, namespace: namespace)
    }
...
struct SlideContainer<Content: View, Theme: ViewModifier>: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .modifier(theme)
            .scaleEffect(min(proxy.size.width / size.width, proxy.size.height / size.height))
...
struct MyTheme: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            ...
            .headerStyle {
                $0.padding(50).border(.white, width: 5)
            }
    }
}
...
struct ImageSlide: View {
    var body: some View {
        Slide(steps: [Animation.easeInOut(duration: 5.0)]) { step in
...
@resultBuilder enum SlideBuilder {
    static func buildBlock<Content>(_ content: Content) -> some SlideList where Content: View {
        Single(view: content)
    }
...
extension EnvironmentValues {
    var headerStyle: AnyViewModifier {
        get {
            self[HeaderStyleKey.self]
        }

        set {
            self[HeaderStyleKey.self] = newValue
        }
    }
}
...
struct ContentView_Previews: PreviewProvider {
    static var previews: some View {
        ForEach(0..<slides.count, id: \.self) { idx in
            SlideContainer(content: AnyView(slides.slide(at: idx)), theme: MyTheme())
                .modifier(PreviewModifier())
                .previewLayout(.fixed(width: 320, height: 180))
                .previewDisplayName("Slide \(idx + 1)")
        }
    }
}
