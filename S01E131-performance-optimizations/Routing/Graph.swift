    ...
    func shortestPath(from: Coordinate, to: Coordinate) -> [Coordinate]? {
        ...
        while let (coordinate, dist) = queue.popLast() {
            ...
            for dest in edges[coordinate] ?? [] where !seen.contains(dest.coordinate) {
                let newDistance = dist + dest.distance
                if let exisitingDist = distances[dest.coordinate]?.distance, exisitingDist < newDistance {
                    continue
                }
                distances[dest.coordinate] = (distance: newDistance, previous: coordinate)
                queue.append((coordinate: dest.coordinate, distance: newDistance))
            }
            //...
