...
struct MatchedGeometryEffect<ID: Hashable>: ViewModifier {
    ...
    var key: GeometryKey { GeometryKey(namespace: namespace, id: id) }
...
extension View {
    func myMatchedGeometryEffect<ID: Hashable>(
        useBuiltin: Bool = true, id: ID, in ns: Namespace.ID, properties: MatchedGeometryProperties = .frame,
...
struct Sample: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            Rectangle()
                .fill(Color.red)
                .myMatchedGeometryEffect(useBuiltin: builtin, id: "ID", in: ns, properties: properties, anchor: anchor)
