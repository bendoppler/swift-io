...
enum Width: Equatable {
    ...
    var min: CGFloat {
        ...
        case .basedOnContents: return 0 // todo log warning or better refactor enum
        }
    }
...
func +(lhs: Layout, rhs: Layout) -> Layout {
    ...
    case let .box(contents: contents, width, wrapper, remainder):
        return .box(contents: contents, width, wrapper: wrapper, remainder + rhs)
        
    case let .space(w, remainder):
        return .space(w, remainder+rhs)
...
class ViewController: UIViewController {
    override func viewDidLoad() {
        ...
        let vertical: Layout = [number, date].vertical(space: 10)
        let layout = [titleLabel,
                      horizontal.or(vertical).box(wrapper: blueBox, width: .flexible(min: 0))
                     ].vertical(space: 20)
