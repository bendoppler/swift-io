...
final class RemoteDebugger<State: Codable>: NSObject, NetServiceBrowserDelegate {
	...
	func startReading() {
		connection?.receive(minimumIncompleteLength: 5, maximumLength: 5, completion: { [unowned self] data, contentContext, isComplete, error in
			guard let d = data, let bytesExpected = decodeJSONHeader(from: d) else {
				print("Error")
				return
			}
			self.connection?.receive(minimumIncompleteLength: bytesExpected, maximumLength: bytesExpected, completion: { [unowned self] data, contentContext, isComplete, error in
				guard let data = data, let result = try? decoder.decode(State.self, from: data) else {
					print("Error content or decode error")
					return
				}
