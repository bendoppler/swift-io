...
extension Path {
    func point(at position: CGFloat) -> CGPoint {
        ...
        return trimmedPath(from: 0, to: position).currentPoint!
    }
    ...
    func pointAndAngle(at position: CGFloat) -> (CGPoint, Angle) {
        ...
        let angle: Angle = Angle(radians: Double(atan2(p2.y - p1.y, p2.x - p1.x)))
        return (p1, angle)
    }
...
struct ContentView: View {
    let rect: some Shape = Rectangle().size(width: 30, height: 30)
    ...
    var body: some View {
        VStack {
            ZStack {
                Eight()
                    .stroke(Color.gray)
                ArrowHead().size(width: 30, height: 30)
                    .offset(x: -15, y: -15)
