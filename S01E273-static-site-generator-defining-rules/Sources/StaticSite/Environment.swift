...
//  
//
//  Created by Do Thai Bao on 24/03/2023.
//
...
extension Rule {
    func environment<Value>(_ keyPath: WritableKeyPath<EnvironmentValues, Value>, _ value: Value) -> some Rule {
        return EnvironmentWritingModifier(content: self) { env in
            env[keyPath: keyPath] = value
        }
    }

    func outputPath(_ path: String) -> some Rule {
        return EnvironmentWritingModifier(content: self, modify: { values in
...
struct Environment<Value>: EnvironmentSettable {
    ...
    var wrappedValue: Value {
        guard let env = values else {
            fatalError("Cannot access the environment outside the body")
        }
