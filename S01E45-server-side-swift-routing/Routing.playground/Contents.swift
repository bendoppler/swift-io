...
struct PathScanner {
    ...
    mutating func constant(_ component: String) -> Bool {
        guard components.first == component else {
            return false
        }
        components.removeFirst()
        return true
...
PathScanner(path: "/test")?.components

extension Route {
    init?(path: String) {
        guard var scanner = PathScanner(path: path) else {
            return nil
        }
        if scanner.isEmpty {
            self = .home
