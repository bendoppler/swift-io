    ...
    public subscript(queryItem name: String) -> String? {
        get {
            guard let items = URLComponents(url: self, resolvingAgainstBaseURL: false)?.queryItems,
                  let index = items.firstIndex(where: { $0.name == name }) else { return nil }
        ...
        set {
            ...
            if let index = components.queryItems?.firstIndex(where: { $0.name == name }) {
...
extension UIView {
    public func constrainEqual(_ attribute: NSLayoutConstraint.Attribute, to: AnyObject, multiplier: CGFloat = 1, constant: CGFloat = 0) {
        constrainEqual(attribute, to: to, attribute, multiplier: multiplier, constant: constant)
    }
    
    public func constrainEqual(_ attribute: NSLayoutConstraint.Attribute, to: AnyObject, _ toAttribute: NSLayoutConstraint.Attribute, multiplier: CGFloat = 1, constant: CGFloat = 0) {
...
extension NSAttributedString {
    ...
    public convenience init(string: String, alignment: NSTextAlignment) {
        ...
        self.init(string: string, attributes: [NSAttributedString.Key.paragraphStyle: style])
    }
}
...
extension Sequence {
    ...
    private func anyIterator() -> AnyIterator<Iterator.Element> {
