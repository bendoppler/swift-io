...
public enum ContentElement {
    case label(String, UIFont.TextStyle, NSTextAlignment)
    case styledLabel(NSAttributedString)
    case textView(NSAttributedString)
    case custom(UIView)
    case button(title: String, callback: () -> ())

    public init(text: String, style: UIFont.TextStyle = .body, alignment: NSTextAlignment = .left) {
...
extension UIStackView {
    public convenience init(axis: NSLayoutConstraint.Axis = .vertical, spacing: CGFloat = 20, distribution: UIStackView.Distribution = .fill, content: [ContentElement]) {
        self.init()
        self.axis = axis
