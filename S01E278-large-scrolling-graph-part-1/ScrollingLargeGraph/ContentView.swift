...
struct DayView: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            Text(day.startOfDay, style: .date).padding(.leading)
        }
...
struct ContentView: View {
    ...
    var centerMostDate: Date? {
        ...
        return visibleDays.mapValues { abs($0 - midX) }.sorted(by: { $0.value < $1.value }).first?.key
    }
            ...
            ScrollView(.horizontal) {
                ScrollViewReader { proxy in
                    LazyHStack(spacing: 0) {
                        ForEach(daysWithNext, id: \.0.id) { (day, nextDay) in
                    ...
                    .onChange(of: date) { newValue in
                        ...
                        let dest = Calendar.current.startOfDay(newValue)
                        withAnimation {
